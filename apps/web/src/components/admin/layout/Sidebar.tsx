...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  useEffect(() => {
    ...
    filteredMenu.forEach(item => {
      if (item.children && item.children.length > 0) {
        ...
        if (hasActiveChild || (pathname.startsWith(item.href) && pathname !== item.href)) {
  ...
  const renderMenuItem = (item: MenuItem, level = 0) => {
    ...
    // 根据层级调整padding
    const paddingClass = level === 0 ? 'px-4' : level === 1 ? 'px-6' : 'px-8'
...
function MenuIcon({ name, className }: { name: string; className?: string }) {
  const icons: Record<string, JSX.Element> = {
    ...
    MegaphoneIcon: (
      <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 4V2a1 1 0 011-1h4a1 1 0 011 1v2m3 6V8a1 1 0 00-1-1H9a1 1 0 00-1 1v2M7 10v10a1 1 0 001 1h8a1 1 0 001-1V10M7 10l1.5-1.5a1 1 0 011.414 0L12 10.5l2.086-2.086a1 1 0 011.414 0L17 10" />
      </svg>
    )
  }

  return icons[name] || icons.CubeIcon
}
